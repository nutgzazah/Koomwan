...
export default function ForumScreen() {
  ...
  const handlePost = async () => {
    ...
    formData.append("title", sanitizedTitle); // คุณสามารถเปลี่ยนให้เป็นค่าจาก TextInput ได้
    ...
    try {
      const response = await axios.post(`${BASE_URL}/api/v1/forum/createPost`, formData);
    ...
    <SafeAreaView className="flex-1">
      ...
      >
        ...
        <Card>
          ...
          <Text className={`w-full right-0 mt-2 pr-4 font-sans text-tag text-right ${charColor}`}>
              เหลือ {remainingChars} อักขระ
            </Text>
          {selectedImage && <Image className="w-full h-[18.75rem] mt-6 rounded-2xl" source={{ uri: selectedImage }} />}
          <BreakLine />
  ...
  function UploadImageButton() {
    return (
      <Pressable className="w-6 h-6 ml-4" onPress={pickImage}>
        <Image className="w-6 h-6" source={require("../../../../assets/Forum/gallery.png")} />
      </Pressable>
    );
  }
      ...
      <>
        <Pressable onPress={handlePost} disabled={isLoading}>
          <View className="flex flex-row bg-primary w-fit h-fit rounded-full justify-evenly items-center px-6 py-2 mr-2">
            ...
            <Image className="ml-2 w-4 h-4" source={require("../../../../assets/Forum/Pen-white.png")} />
          </View>
        </Pressable>
