  ...
  return (
    <View className="mb-2">
      <TouchableOpacity
        ...
        style={dropdownStyle}
      >
        <Text className="text-description font-sans flex-1 text-gray">
          {selectedChoice || "เลือกประเภท"}
        </Text>
        <Image
      ...
      {isDropdownOpen && (
        <View className="mt-2 bg-white border border-gray rounded-lg">
          {choices.map((choice) => (
            ...
            >
              <Text
                className={`text-description font-sans text-secondary ${
                  selectedChoice === choice
                    ? "text-description font-sans bg-primary text-white p-1"
                    : ""
                }`}
              >
                {choice}
      ...
      {selectedChoice === "อื่นๆ" && (
        <TextInput
          ...
          className="text-description font-sans border border-gray rounded-lg p-2 mt-2"
        />
      )}
    </View>
  );
